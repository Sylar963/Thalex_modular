...
class BybitHistoryPrefetcher(IHistoryPrefetchService):
    __slots__ = ("_provider", "_lookback_days")

    def __init__(self, db_adapter: TimescaleDBAdapter, lookback_days: int = 15):
    ...
    async def prefetch(self, symbol: str) -> int:
        ...
        config = HistoryConfig(symbol=symbol, start_time=start_ts, end_time=now)
