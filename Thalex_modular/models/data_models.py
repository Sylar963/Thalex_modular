...
import thalex as th
from ...legacy_quoter_files.keys import *  # Rename _keys.py to keys.py and add your keys. There are instructions how to create keys in that file.
...
class Ticker:
    def __init__(self, data: Dict):
        ...
        self.volume: Optional[float] = data.get("volume", 0.0)
        self.open_interest: Optional[float] = data.get("open_interest", 0.0)
...
class Quote:
    ...
    def to_side_quote(self) -> th.SideQuote:
        return th.SideQuote(price=self.price, amount=self.amount)

    @classmethod
    def from_side_quote(cls, side_quote: th.SideQuote) -> 'Quote':
        return cls(price=side_quote.p, amount=side_quote.a)
